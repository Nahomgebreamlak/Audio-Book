...
class PDFScreen extends StatefulWidget {
  final String path;
  int pagenumber;
  PDFScreen({Key key, this.path, this.pagenumber}) : super(key: key);
...
class _PDFScreenState extends State<PDFScreen> with WidgetsBindingObserver {
      ...
      body: Stack(
        children: <Widget>[
          PDFView(
            ...
            defaultPage: box.read('pagenumber'),
            fitPolicy: FitPolicy.BOTH,
            preventLinkNavigation:
                false, // if set to true the link is handled in flutter
            onRender: (_pages) {
              viewcontroler.setPagenumber();
              setState(() {
                //  pages.value = _pages;
                pages.value = viewcontroler.pagenumber.value;
            ...
            onPageChanged: (int page, int total) {
              ...
              setState(() {
                ...
                box.write("pagenumber", currentPage.value);
                print("bax value ${box.read('pagenumber')}");
  ...
  get floatingActionItem {
    Widget floatingPlayer = FloatingAudioPlayer(
        onPaused: onPaused,
        audiopath: "assets/chapter1.m4a",
        musicduration: "musicplayerch1");
